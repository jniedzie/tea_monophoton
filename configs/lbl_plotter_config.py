...
for prefix in ["", "Barrel_", "EndCap_"]:
  for prefixExtra in ["", "timingSR_", "timingCR_"]:
    histograms += (
      #           name                  title logx logy    norm_type                    rebin xmin   xmax  ymin    ymax,    xlabel                ylabel            suffix
      Histogram(f"goodPhoton_{prefix}{prefixExtra}et", "", False, True, default_lumi, 5, 0, 100, 1e-2, 5e5, "E_{T}^{#gamma} (GeV)", y_label, "", lbl_error),
      Histogram(f"goodPhoton_{prefix}{prefixExtra}eta", "", False, True, default_lumi, 1, -3, 3, 1e-2, 5e5, "#eta^{#gamma}", y_label, "_log", lbl_error),
      Histogram(f"goodPhoton_{prefix}{prefixExtra}eta", "", False, False, default_lumi, 1, -3, 3, 0, 50, "#eta^{#gamma}", y_label, "", lbl_error),
      
      Histogram(f"goodPhoton_{prefix}{prefixExtra}phi", "", False, False, default_lumi, 4, -4, 4, 1e-2, 50, "#phi^{#gamma}", y_label, "", lbl_error),
      Histogram(f"goodPhoton_{prefix}{prefixExtra}seedTime", "", False, True, default_lumi, 10, -30, 30, 1e-2, 5e6, "Photon seed time (ns)", y_label, "", lbl_error),
      
      Histogram(f"goodPhoton_{prefix}{prefixExtra}hOverE", "", False, True, default_lumi, 1, 0, 0.025, 1e-2, 5e5, "H/E", y_label, "", lbl_error),
      Histogram(f"goodPhoton_{prefix}{prefixExtra}swissCross", "", False, True, default_lumi, 20, 0, 1.5, 1e-2, 5e10, "Swiss cross", y_label, "", lbl_error),
